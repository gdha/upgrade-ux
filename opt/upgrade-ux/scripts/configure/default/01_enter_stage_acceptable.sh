...
# We need to verify before entering this stage of CURRENT_STATUS="postremove:ended"
...
if [[ "$CURRENT_STATUS" == "postremove:ended" ]]; then
    # nice we will now update our stage to "configure:start"
    CURRENT_STATUS="$stage:start"
    SetCurrentStatus "$STATUS_FILE"  # write it to our STATUS_FILE
elif [[ "$CURRENT_STATUS" == "configure:start" ]]; then
