...
# We need to verify before entering this stage of CURRENT_STATUS="preinstall:ended"
...
if [[ "$CURRENT_STATUS" = "preinstall:ended" ]]; then
    # nice we will now update our stage to "install:start"
    CURRENT_STATUS="$stage:start"
    SetCurrentStatus "$STATUS_FILE"  # write it to our STATUS_FILE
elif [[ "$CURRENT_STATUS" = "install:start" ]]; then
