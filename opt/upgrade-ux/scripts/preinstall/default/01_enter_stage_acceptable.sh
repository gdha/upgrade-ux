...
# We need to verify before entering this stage of CURRENT_STATUS="preremove:ended"
...
if [[ "$CURRENT_STATUS" == "preremove:ended" ]]; then
    # nice we will now update our stage to "preinstall:start"
    CURRENT_STATUS="$stage:start"
    SetCurrentStatus "$STATUS_FILE"  # write it to our STATUS_FILE
elif [[ "$CURRENT_STATUS" == "preinstall:start" ]]; then
