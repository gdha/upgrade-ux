...
# We need to verify before entering this stage of CURRENT_STATUS="configure:ended"
...
if [[ "$CURRENT_STATUS" == "configure:ended" ]]; then
    # nice we will now update our stage to "postexecute:start"
    CURRENT_STATUS="$stage:start"
    SetCurrentStatus "$STATUS_FILE"  # write it to our STATUS_FILE
elif [[ "$CURRENT_STATUS" == "postexecute:start" ]]; then
